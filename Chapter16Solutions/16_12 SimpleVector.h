...
template <>
class SimpleVector<std::string> {
private:
	std::string* aptr; // To point to the allocated array 
	int arraySize; // Number of elements in the array 
	void memError(); // Handles memory allocation errors 
	void subError(); // Handles subscripts out of range 
...
void SimpleVector<std::string>::push_back(std::string v) {
	...
	{
		//create a new array that is temporary and will be +1 larger than the original array;
		std::string* newArr = new std::string[array_new_size];
		...
		aptr = new std::string[array_new_size];
...
std::string& SimpleVector<std::string>::operator[](const int& sub)
{
	if (sub < 0 || sub >= arraySize)
		subError();
	return aptr[sub];
...
void SimpleVector<std::string>::memError()
{
	cout << "ERROR:Cannot allocate memory.\n";
	exit(EXIT_FAILURE);
}
...
void SimpleVector<std::string>::subError()
{
	cout << "ERROR: Subscript out of range.\n";
	exit(EXIT_FAILURE);
}

SimpleVector<std::string>::SimpleVector(int s)
{
	arraySize = s;
	// Allocate memory for the array. 
	try
	{
		aptr = new std::string[s];
	}
	catch (bad_alloc)
	{
		memError();
...
SimpleVector<std::string>::SimpleVector(const SimpleVector& obj)
{
	// Copy the array size. 
	arraySize = obj.arraySize;

	// Allocate memory for the array. 
	aptr = new std::string[arraySize];
	if (aptr == 0)
		memError();
...
SimpleVector<std::string>::~SimpleVector()
{
	if (arraySize > 0) {
		delete[] aptr;
	}
...
std::string SimpleVector<std::string>::getElementAt(int sub)
{
	if (sub < 0 || sub >= arraySize)
		subError();
	return aptr[sub];
